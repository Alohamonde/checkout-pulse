...
const DISCOUNT_TITLE = "Checkout Pulse Upsell";

const LIST_DISCOUNTS = `#graphql
  query UpsellDiscountList {
...
const CREATE_DISCOUNT = `#graphql
  mutation UpsellDiscountCreate($automaticAppDiscount: DiscountAutomaticAppInput!) {
...
const UPDATE_DISCOUNT = `#graphql
  mutation UpsellDiscountUpdate($id: ID!, $automaticAppDiscount: DiscountAutomaticAppInput!) {
...
function isOurDiscount(discount: {
  ...
  return (discount.title || "").includes("Checkout Pulse");
}

export async function getUpsellDiscountStatus(admin: Parameters<typeof safeGraphql>[0]) {
  ...
  const match = nodes.find((node: { automaticDiscount: unknown }) =>
    isOurDiscount(node.automaticDiscount as { title?: string; appDiscountType?: { appKey?: string } }),
  );
...
async function repairDiscount(admin: Parameters<typeof safeGraphql>[0], id: string) {
...
export async function ensureUpsellDiscount(admin: Parameters<typeof safeGraphql>[0]) {
  const current = await getUpsellDiscountStatus(admin);
  ...
  if (current.exists) {
    ...
    const after = await getUpsellDiscountStatus(admin);
  ...
  const created = createResult.data?.discountAutomaticAppCreate?.automaticAppDiscount;
